...
const QUOTES = {
  ...
  feed: ["Thomai hat sich selbst übertroffen!", "Boah, Thomai kocht wie ein Gott!", "Nom nom... Thomai for President!"],
  fat: ["Mäste mich nicht!", "Ich rolle sonst bald!", "Ich bin satt!"],
  workStart: ["Ab ins Büro... argh.", "Karriere ruft!", "Zeit ist Gold."],
...
// ERWEITERTE KOSTÜMLISTE (7 XP-Skins + Shop-Skins)
const COSTUMES = [
  ...
  { id: 'ninja', name: 'Schatten-Ninja', price: 0, lvl: 6, color: '#1a1a1a' },
  ...
  { id: 'pirate', name: 'Captain Hami', price: 100, lvl: 0, color: '#3E2723' },
  { id: 'astronaut', name: 'Hami-naut', price: 150, lvl: 0, color: '#CFD8DC' },
  ...
  return (
    <motion.div animate={isWorking ? { x: [-5, 5, -5], rotate: [-1, 1, -1] } : { y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 2.5 }}>
      <svg width="220" height="220" viewBox="0 0 200 200" className="drop-shadow-2xl">
        ...
        {id === 'ninja' && <g><rect x="45" y="75" width="110" height="25" fill="#333" /><circle cx="75" cy="87" r="11" fill="white" opacity="0.2" /><circle cx="125" cy="87" r="11" fill="white" opacity="0.2" /></g>}
        ...
        {id === 'king' && <path d="M 65 55 L 75 35 L 100 55 L 125 35 L 135 55 Z" fill="#FFD700" stroke="#B8860B" />}
...
export default function App() {
  ...
  const startGame = () => {
    ...
    setGameActive(true); setGameScore(0); setSeeds([]); setGameResult(null); setGameTime(30); setLastGame(Date.now());
  };
        ...
        <div className="w-full md:w-2/3 aspect-square bg-white rounded-[4rem] shadow-2xl relative border-8 border-white overflow-hidden flex items-center justify-center bg-gradient-to-b from-orange-50 to-white">
          ...
          {isWorking ? (
            <div className="text-center">
              ...
              <button onClick={() => {if(window.confirm("Abbrechen? Keine XP, kein Gold!")){setIsWorking(false); setWorkLeft(0); say(QUOTES.workCancel)}}} className="mt-4 bg-red-100 text-red-600 px-6 py-2 rounded-2xl font-black uppercase text-xs flex items-center gap-2 shadow-sm"><AlertTriangle size={14}/> Abbruch</button>
            </div>
          ) : (
                  ...
                  return (
                    <button 
                      ...
                      onClick={() => {
                        if(canUse){
                          setCurrentId(c.id);
                        } else if(c.lvl === 0 && coins >= c.price){
                          setCoins(coins-c.price); setUnlocked([...unlocked, c.id]); setCurrentId(c.id);
                        } else if (c.lvl > 0 && level < c.lvl) {
                          say([`Level ${c.lvl} benötigt! Ich bin noch nicht bereit!`]);
                        }
                      }} 
                    ...
                    >
                      {c.name} 
                      {!canUse && (
                        <div className="text-[8px] text-amber-600 flex items-center justify-center gap-1">
                          {c.lvl > 0 ? <><Lock size={8}/> LVL {c.lvl}</> : <><Coins size={8}/> {c.price}</>}
                        </div>
                      )}
                    </button>
                  );
        ...
        {gameActive && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-sky-400 z-[200] p-8 flex flex-col items-center">
             <div className="flex justify-between w-full max-w-2xl mb-4">
                 <h2 className="text-4xl font-black text-white italic uppercase">KERNE: {gameScore}/40</h2>
                 <h2 className="text-4xl font-black text-white italic uppercase">ZEIT: {gameTime}S</h2>
             </div>
               ...
               <AnimatePresence>
                 {seeds.map(s => (
                   <motion.div 
                    ...
                    animate={{y:1000}} 
                    exit={{opacity:0}}
                    onClick={()=>{if(gameScore<40){setGameScore(s=>s+1); setSeeds(o=>o.filter(x=>x.id!==s.id))}}} 
        ...
        {gameResult !== null && (
            <motion.div initial={{opacity:0, scale:0.5}} animate={{opacity:1, scale:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/80 z-[250] flex items-center justify-center p-8">
                <div className="bg-white p-10 rounded-[3rem] text-center shadow-2xl border-8 border-orange-400">
                    ...
                    <p className="text-2xl font-bold mb-2 tracking-tight">Du hast <span className="text-orange-500">{gameResult}</span> Kerne gefangen!</p>
                    <p className="text-xl text-green-600 font-black mb-8 italic">+{gameResult} XP erhalten</p>
                    <button onClick={() => setGameResult(null)} className="bg-orange-500 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg active:scale-95">Super!</button>
                </div>
            </motion.div>
        )}
      </AnimatePresence>

      <button onClick={resetProgress} className="fixed bottom-2 left-2 opacity-10 hover:opacity-100 p-2 text-red-600 transition-all z-[100] flex items-center gap-1 text-[10px] font-black italic">
